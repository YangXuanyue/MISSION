...
fast_parser::fast_parser(const char *name) : status(true), newline(false),
                                             offset(0), idx(0), count(0),
                                             addr(nullptr), taddr(nullptr) {
...
bool fast_parser::initialize_mmap() {
    addr = mmap(NULL, pg_size, PROT_READ, MAP_FILE | MAP_PRIVATE | MAP_POPULATE,
                fd, offset);
    if (addr == MAP_FAILED) {
...
data_t fast_parser::strtok(const char delimiter) {
    ...
    size_t cdx = 0;
    for (char v = this->operator++(0);
         status && v != delimiter && v != '\n'; v = this->operator++(0)) {
        buf[cdx++] = v;
    }
    return buf;
}
