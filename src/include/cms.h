...
public:
    ...
    float update(const void *key, const int len, float value) {
        ...
        for (size_t idx = 0; idx < N; ++idx) {
            ...
            const bool sign_bit =
                MurmurHash3_x86_32(key, len, seeds[idx] + 3) & 0x1;
    ...
    void cms_update(const hc<N> &cache, const size_t cdx, __m256 value) {
        for (size_t idx = 0; idx < N; ++idx) {
            ...
            if (cdx < DIV) {
                ...
                __m256 result = _mm256_add_ps(current,
                                              _mm256_mul_ps(sign, value));
     ...
     */
    float
    cms_retrieve_single(const hc<N> &cache, const size_t class_idx) const {
        std::vector<float> values(N, 0);
        for (size_t idx = 0; idx < N; ++idx) {
